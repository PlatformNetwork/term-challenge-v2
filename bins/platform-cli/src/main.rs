...
enum Commands {
    ...
    Config,
    /// Deploy a WASM challenge module to a validator's module directory
    Deploy {
        /// Challenge ID (used as the WASM filename, e.g. "my-challenge" → "my-challenge.wasm")
        #[arg(long)]
        challenge_id: String,
        /// Path to the compiled WASM module file
        #[arg(long)]
        wasm_path: PathBuf,
        /// Target directory where the validator loads WASM modules from
...
fn cmd_deploy(challenge_id: &str, wasm_path: &Path, module_dir: &Path) -> Result<()> {
    if challenge_id.is_empty() {
        anyhow::bail!("Challenge ID must not be empty");
    }
    if challenge_id.contains('/') || challenge_id.contains('\\') || challenge_id.contains("..") {
        anyhow::bail!(
            "Invalid challenge ID '{}': must not contain path separators or '..'",
            challenge_id
        );
    }

    let wasm_bytes = std::fs::read(wasm_path)
        .with_context(|| format!("Failed to read WASM file from {}", wasm_path.display()))?;

    if wasm_bytes.len() < 4 || wasm_bytes[..4] != WASM_MAGIC {
        anyhow::bail!(
            "File {} does not appear to be a valid WASM module (missing \\0asm magic header)",
            wasm_path.display()
        );
    }
    ...
    let dest = module_dir.join(format!("{}.wasm", challenge_id));
    std::fs::write(&dest, &wasm_bytes)
        .with_context(|| format!("Failed to write WASM module to {}", dest.display()))?;
    ...
    info!(
        ...
        sha256 = %hash,
        dest = %dest.display(),
        "Challenge WASM module deployed"
    );

    println!(
        "✓ Deployed challenge '{}' ({} bytes, sha256:{}) to {}",
        challenge_id,
        wasm_bytes.len(),
        &hash[..16],
        dest.display()
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Deploy {
            ...
            wasm_path,
            module_dir,
        } => cmd_deploy(&challenge_id, &wasm_path, &module_dir),
    }
}
