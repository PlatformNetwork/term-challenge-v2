...
async fn main() -> Result<()> {
    ...
    let is_bootnode = args.bootstrap.is_empty();
    if is_bootnode {
        warn!(
            "No bootstrap peers configured. Starting in bootstrap mode — \
             this node will listen for incoming P2P connections."
        );
    }
            ...
            P2PMessage::ChallengeUpdate(update) => {
                    ...
                    || update.updater.0 == platform_core::SUDO_KEY_BYTES
                {
                    ...
                    if !update.data.is_empty() {
                        let wasm_path =
                            wasm_module_dir.join(format!("{}.wasm", update.challenge_id));
                        match std::fs::write(&wasm_path, &update.data) {
                            Ok(()) => {
                                info!(
                                    challenge_id = %update.challenge_id,
                                    path = %wasm_path.display(),
                                    size_bytes = update.data.len(),
                                    "Persisted WASM challenge binary to disk"
                                );
                            }
                            Err(e) => {
                                error!(
                                    ...
                                    path = %wasm_path.display(),
                                    error = %e,
                                    "Failed to persist WASM challenge binary"
                                );
                            }
                        }
                    }
