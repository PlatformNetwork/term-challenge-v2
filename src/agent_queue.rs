...
impl EvalRequest {
    pub fn new(agent: QueueAgentInfo, miner_hotkey: String, miner_uid: u16, miner_stake: u64, epoch: u64) -> Self {
        Self {
...
impl ResourceManager {
    ...
    fn register_container(&self, container_id: &str) {
        self.active_containers.write().insert(container_id.to_string());
    }
            ...
            Ok(containers) => {
                for container in containers {
                    if let Some(id) = container.id {
                        ...
                        if self.docker.remove_container(&id, Some(rm_options)).await.is_ok() {
...
impl AgentQueue {
    ...
    pub async fn submit(&self, request: EvalRequest) -> Result<String> {
        ...
        info!(
            ...
            request.agent.hash, request.miner_hotkey, request.miner_stake, pending.len() + 1
        );
    ...
    pub fn stats(&self) -> QueueStats {
        ...
        QueueStats {
            ...
            active_tasks: MAX_GLOBAL_CONCURRENT_TASKS - self.resources.task_semaphore.available_permits(),
    ...
    fn calculate_concurrent_tasks(&self) -> usize {
        ...
        // Clamp to min/max
        per_agent.max(MIN_TASKS_PER_AGENT).min(MAX_TASKS_PER_AGENT)
    }
    
    /// Start the queue processor
    pub async fn run(self: Arc<Self>) {
        info!("Starting agent queue processor (max {} concurrent tasks)", MAX_GLOBAL_CONCURRENT_TASKS);
        ...
        loop {
            ...
            tokio::spawn(async move {
                queue.run_evaluation(request, concurrent_tasks, resources).await;
            });
    ...
    async fn run_evaluation(
        ...
        // Register as running
        {
            ...
            running.insert(request_id.clone(), RunningEval {
    ...
    async fn evaluate_task(task: &Task, agent: &QueueAgentInfo, container_name: &str) -> TaskEvalResult {
        let start = Instant::now();
    ...
    fn complete_with_error(&self, request: &EvalRequest, error: &str) {
        ...
        // Store result
        self.results.write().insert(request.id.clone(), result.clone());
    ...
    async fn complete_evaluation(&self, result: EvalResult) {
        ...
        // Store result
        self.results.write().insert(request_id.clone(), result.clone());
