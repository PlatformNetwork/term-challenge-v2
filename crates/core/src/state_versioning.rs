...
pub const CURRENT_STATE_VERSION: u32 = 7;
...
fn migrate_state(version: u32, data: &[u8]) -> Result<crate::ChainState> {
    match version {
        ...
        3 => {
            // V3 -> V7: Add wasm_challenge_configs
            ...
            info!(
                "Migrated state V3->V7: block_height={}, validators={}",
                v3.block_height,
                v3.validators.len()
            );
            Ok(v3)
        ...
        4 => {
            // V4 -> V7: Added WASM restart metadata and removed docker configs
            ...
            info!(
                "Migrated state V4->V7: block_height={}, validators={}",
                v4.block_height,
                v4.validators.len()
            );
            Ok(v4)
        ...
        5 => {
            // V5 -> V7: Remove docker configs (handled by serde defaults)
            ...
            info!(
                "Migrated state V5->V7: block_height={}, validators={}",
                v5.block_height,
                v5.validators.len()
            );
            Ok(v5)
        ...
        6 => {
            // V6 -> V7: Added network_stopped, network_stop_reason (handled by serde defaults)
            let v6: crate::ChainState = bincode::deserialize(data).map_err(|e| {
                crate::MiniChainError::Serialization(format!("V6 migration failed: {}", e))
            })?;
            info!(
                "Migrated state V6->V7: block_height={}, validators={}",
                v6.block_height,
                v6.validators.len()
            );
            Ok(v6)
...
mod tests {
    ...
    fn test_version_constants() {
        ...
        assert_eq!(CURRENT_STATE_VERSION, 7);
    }
