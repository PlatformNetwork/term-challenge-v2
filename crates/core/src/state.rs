...
use crate::{
    hash_data, BlockHeight, Challenge, ChallengeConfig, ChallengeId, ChallengeWeightAllocation,
...
pub struct ChainState {
    ...
    /// Whether the network is stopped (all emissions go to UID 0 burn)
    #[serde(default)]
    pub network_stopped: bool,

    /// Reason for network stop
    #[serde(default)]
    pub network_stop_reason: Option<String>,
}
...
impl ChainState {
    ...
    /// Add a challenge from a sudo action
    pub fn add_challenge_from_sudo(
        &mut self,
        name: String,
        description: String,
        wasm_code: Vec<u8>,
        ...
        let challenge = Challenge::new(name, description, wasm_code, owner, config);
        let id = challenge.id;
        let wasm_config = WasmChallengeConfig::from(&challenge);
        self.challenges.insert(id, challenge);
        self.wasm_challenge_configs.insert(id, wasm_config);
        if weight > 0 {
            let allocation = ChallengeWeightAllocation::new(id, 0, weight as f64 / 100.0);
            self.challenge_weights.insert(id, allocation);
        }
        self.update_hash();
        id
    }

    /// Remove a challenge from a sudo action
    pub fn remove_challenge_from_sudo(&mut self, challenge_id: &ChallengeId) -> bool {
        ...
        if let Some(wasm_config) = self.wasm_challenge_configs.get_mut(challenge_id) {
            wasm_config.is_active = false;
        }
        if let Some(allocation) = self.challenge_weights.get_mut(challenge_id) {
    ...
    /// Edit a challenge from a sudo action
    pub fn edit_challenge_from_sudo(
        &mut self,
        ...
        if let Some(n) = name {
            challenge.name = n.clone();
            if let Some(wc) = self.wasm_challenge_configs.get_mut(challenge_id) {
                wc.name = n;
            }
        }
        if let Some(d) = description {
            challenge.description = d.clone();
            if let Some(wc) = self.wasm_challenge_configs.get_mut(challenge_id) {
                wc.description = d;
            }
        }
        if let Some(code) = wasm_code {
            challenge.update_code(code);
            if let Some(wc) = self.wasm_challenge_configs.get_mut(challenge_id) {
                wc.module.code_hash = challenge.code_hash.clone();
            }
        }
        if let Some(c) = config {
            challenge.config = c.clone();
            if let Some(wc) = self.wasm_challenge_configs.get_mut(challenge_id) {
                wc.config = c;
            }
        }
        if let Some(w) = weight {
            if let Some(allocation) = self.challenge_weights.get_mut(challenge_id) {
                allocation.weight_ratio = w as f64 / 100.0;
            } else {
                let allocation = ChallengeWeightAllocation::new(*challenge_id, 0, w as f64 / 100.0);
    ...
    /// Stop the network - all emissions go to UID 0 (burn)
    pub fn stop_network(&mut self, reason: String) {
        self.network_stopped = true;
        self.network_stop_reason = Some(reason);
        self.update_hash();
    }
